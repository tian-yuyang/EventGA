...
from generative_agent_high_level import GenerativeAgent
...
for agent_filename in agent_filenames:
    agent_kwargs = {"path":f"{input_folder_name}/{agent_filename}"}
    #inplace dict update
...
records = {}
time_slice_records = {}
if(os.path.exists(f"{input_folder_name}/records.json")):
    records = load_json_file(f"{input_folder_name}/records.json")
...
while agents:
    ...
    if curr_time <= simulation_time:
        ...
        output_filename = f"{output_folder_name}/state_{agent.name}_{DatetimeNL.get_time_nl(curr_time).replace(':','_')}.json"
        print(output_filename)
        write_json_file(overall_log, output_filename)
    else:
        output_filename = f"{output_folder_name}/{DatetimeNL.get_time_nl(simulation_time).replace(':','_')}.json"
