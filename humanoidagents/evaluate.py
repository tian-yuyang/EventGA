...
# 解析命令行参数
parser = argparse.ArgumentParser(description='运行人形代理模拟评估')
parser.add_argument("-c", "--config_filename", default="../simulation_args/fitness.json")
args = parser.parse_args()
args = load_json_file(args.config_filename)
...
def get_prompt(agent, criterion, history):
    ...
    if criterion == "Role Fulfillment":
        ...
        return f"Evaluate whether {agent} was at Oak Hill College during the competition hours from 11 am to 2 pm. Rate on a scale of 1 to 10, with 1 being not present at all and 10 being present throughout the entire period. Here is {agent}'s action history:\n{history}\nPlease provide your answer as a single number between 1 and 10."
        ...
        return f"Evaluate the quality of {agent}'s interactions with other agents related to the fitness competition. Consider whether they communicated effectively, coordinated actions, and collaborated as needed for their role. Rate on a scale of 1 to 10, with 1 being poor interaction and 10 being excellent interaction. Here is {agent}'s action history:\n{history}\nPlease provide your answer as a single number between 1 and 10."
    else:
...
def check_history(folder):
    ...
    end_time = datetime.strptime("10:00:00 pm", "%I:%M:%S %p")
    time_increment = timedelta(seconds=5)
    current_time = start_time
